...
const ClassroomGrid = ({ localStream, user, onLeave }) => {
  ...
  useEffect(() => {
    ...
    socket.on('answer', (payload) => {
      const item = peersRef.current.find(p => p.peerID === payload.responderId);
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6 overflow-y-auto scrollbar-hide pb-24">
        <VideoPlayer stream={localStream} isLocal={true} username={user.username} role={user.role} />
      ...
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex items-center gap-4 bg-background-paper/90 backdrop-blur-lg border border-white/10 px-6 py-3 rounded-2xl shadow-2xl z-50">
        ...
        <ControlButton 
          onClick={toggleMic} 
          isActive={micOn}
          icon={micOn ? <MicrophoneIcon className="w-6 h-6"/> : <MicrophoneSlashIcon className="w-6 h-6"/>} 
        />

        <ControlButton 
          onClick={toggleCam} 
          isActive={camOn}
          icon={camOn ? <VideoCameraIcon className="w-6 h-6"/> : <VideoCameraSlashIcon className="w-6 h-6"/>} 
        />
...
const VideoWrapper = ({ peer }) => {
  ...
  if (!stream) return (
    <div className="aspect-video bg-slate-900/50 rounded-xl border border-white/5 animate-pulse flex items-center justify-center">
      <span className="text-xs text-slate-500 tracking-widest uppercase">Connecting...</span>
    </div>
  );
